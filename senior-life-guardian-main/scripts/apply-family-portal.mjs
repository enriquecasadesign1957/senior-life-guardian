/**
 * Aplica bootstrap Portal Familia en Supabase producción.
...
const sqlPath = path.resolve(
  process.cwd(),
  "supabase/migrations/20260622120000_bootstrap_family_portal_production.sql",
);
...
for (const { host, port, user } of targets) {
  ...
  try {
    ...
    await client.end();
    console.log(`family_portal OK (${user}@${host}:${port}).`);
    process.exit(0);
  } catch (e) {
    lastError = e;
