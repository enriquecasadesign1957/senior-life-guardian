  ...
  .handler(async ({ data }) => {
    ...
    if (!member) {
      ...
      const { data: ec } = await supabaseAdmin
        ...
        .select("trial_signup_id, nombre, parentesco")
      ...
      const { data: created, error: cErr } = await supabaseAdmin
        .from("family_members")
        .insert({
          trial_signup_id: ec.trial_signup_id,
          nombre: ec.nombre,
          telefono: tel,
...
async function sendCode(memberId: string, tel: string) {
  ...
  const okWA = await twilioSend(tel, body, "whatsapp");
  if (!okWA) await twilioSend(tel, body, "sms");
...
export const verifyFamilyCode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await logAccess(member.id, member.trial_signup_id, "login_success", { telefono: tel });

    return {
      ok: true,
      session: {
        family_member_id: member.id,
        trial_signup_id: member.trial_signup_id,
...
const sessionSchema = z.object({
  ...
  trial_signup_id: z.string().uuid(),
});

async function assertSession(familyMemberId: string, signupId: string) {
  const { data } = await supabaseAdmin
    .from("family_members")
    .select("id, trial_signup_id, activo")
    .eq("id", familyMemberId)
    .maybeSingle();
  if (!data || !data.activo || data.trial_signup_id !== signupId) {
...
export const getFamilyDashboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await assertSession(data.family_member_id, data.trial_signup_id);

    const [{ data: senior }, { data: device }, { data: alerts }] = await Promise.all([
      supabaseAdmin
        .from("trial_signups")
        .select("id, nombre, telefono")
        .eq("id", data.trial_signup_id)
        .maybeSingle(),
      supabaseAdmin
        .from("device_status")
    ...
    await logAccess(data.family_member_id, data.trial_signup_id, "dashboard_view");
...
export const ackAlertByToken = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { data: alert } = await supabaseAdmin
      .from("alert_logs")
      .select("id, acknowledged_at, acknowledgement_expires_at, trial_signup_id")
    ...
    return { ok: true, already: false, trial_signup_id: alert.trial_signup_id };
  });
